...
class Telegram(RPCHandler):
    ...
    def _init(self) -> None:
        ...
        if self._config["telegram"].get("noti_token"):
            self._bot_noti = Bot(token=self._config["telegram"]["noti_token"])
    ...
    def send_msg(self, msg: RPCSendMsg) -> None:
        ...
        if message:
            ...
            if noti == "silent":
                logger.info(f"Notification '{msg['type']}' not sent to notification channel.")
                return

            if self._bot_noti:
                asyncio.run_coroutine_threadsafe(
                    self._send_msg_noti(message), self._loop)
    ...
    async def _send_msg_noti(
        ...
        """
        Send given markdown message to notification channel
        :param msg: message
        :param parse_mode: telegram parse mode
        :param disable_notification: whether to send the message silently
        :return: None
        """
        if self._bot_noti and self._config["telegram"].get("noti_chat_id"):
            try:
                try:
                    await self._bot_noti.send_message(
                        self._config["telegram"]["noti_chat_id"],
                        text=msg,
                        parse_mode=parse_mode,
                        disable_notification=disable_notification,
                        message_thread_id=self._config["telegram"].get("noti_topic_id"),
                    )
                except NetworkError as network_err:
                    # Sometimes the telegram server resets the current connection,
                    # if this is the case we send the message again.
